...
*/
	...
	*/
	...
	public function readSMS()
	{
		...
		// Read pdu
		do{
			if(!$this->_socket) return false; //проверяем сокет
			...
			if($pdu->id==SMPP::ENQUIRE_LINK) {
				...
				array_push($this->pdu_queue, $pdu);  //это фигачит левые смски
			}
	...
	protected function readPDU()
	{
		...
		if($len>0){
			...
			if(!$body) $body = null; //throw new RuntimeException('Could not read PDU body');
		} else {
			$body=null;
		}
	...
	{
		// Check command id
		if($pdu->id != SMPP::DELIVER_SM) return; //throw new InvalidArgumentException('PDU is not an received SMS');
...
// private members (not documented):
	...
	protected function parseTag(&$ar)
	{
		...
		if (!$unpackedData) return ; //throw new InvalidArgumentException('Could not read tag data');
...
class SmppDeliveryReceipt extends SmppSms
{
	...
	public function parseDeliveryReceipt()
	{
		...
		$numMatches = preg_match('/^id:([^ ]+) sub:(\d{1,3}) dlvrd:(\d{3}) submit date:(\d{10}) done date:(\d{10}) stat:([A-Z]{7}) err:(\d{1,3})(?:\stext:(.*))?/ms', $this->message, $matches); //PATCH by iZonder: не все сообщения имеют поле text, не нужно вызывать exception
		if ($numMatches == 0) {
