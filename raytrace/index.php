...
$p = array('x' => 4, 'y' => 8);
$f = pi()/20; //берем шаговый угол 1/20*PI rad ~ 9 grad
...
for($s = 1; $s < 10; $s++) { //делаем 9 итераций (при 10 итерации угол будет равен 90 градусов, а тангенс соответсвенно неопределен)
	...
	for($i = 0; $i < $c; $i++) {
		...
		$img->vector($img->scaleX($v[$i]['x']), $img->scaleY($v[$i]['y']), $img->scaleX($v[$j]['x']), $img->scaleY($v[$j]['y']), imagecolorallocate($img->img, 0, 0, 255));
	...
	if(isset($vote[$intersect])) ++$vote[$intersect];
	else $vote[$intersect] = 1;
	//if($intersect == 1) $img->num($img->scaleX($x), $img->scaleY($y), "({$x}; {$y})");
}

print_r($vote);
$intersect = array_search(max($vote), $vote);
	...
	function __construct($x, $y, $chains)
	{
		...
		if(count($chains) > 0) {
			foreach($chains as $chain) {
				foreach($chain as $v) {
					$this->vector($this->scaleX($v['x1']), $this->scaleY($v['y1']), $this->scaleX($v['x2']), $this->scaleY($v['y2']), $grey);
				}
				$this->num($this->scaleX($v['x2']), $this->scaleY($v['y2']), $v['chain'], $red);
			}
		}
	}
