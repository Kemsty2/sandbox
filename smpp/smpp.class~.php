...
*/
		$this->_socket = fsockopen($host, $port, $errno, $errstr, 20); //var_dump($this->_socket); die();
	...
	*/
	function Send($to, $text, $unicode = false,$time = "",$flash = false)
	{
		...
		$sm_default_msg_id = 0;
		if ($unicode) {
			$text = mb_convert_encoding($text, "UCS-2BE", "UTF-8,HTML-ENTITIES"); /* UCS-2BE */
			$data_coding = 8; /* UCS2 */
	...
	*/
	function SendMulti($tolist, $text, $unicode = false)
	{
		...
		$sm_default_msg_id = 0;
		if ($unicode) {
			$text = mb_convert_encoding($text, "UCS-2BE", "UTF-8,HTML-ENTITIES");
			$data_coding = 8; /* UCS2 */
			$multi = $this->split_message_unicode($text);
		}
		else {
